...
function loadObj(objPath) {
    return new Promise(function(resolve, reject) {
        ...
        var stream = byline(fsExtra.createReadStream(objPath, {encoding: 'utf8'}));
...
function getImagePaths(materials) {
    ...
    for (var name in materials) {
        if (materials.hasOwnProperty(name)) {
            ...
            if (defined(material.ambientColorMap) && imagePaths.indexOf(material.ambientColorMap) >= 0) {
                imagePaths.push(material.ambientColorMap);
            }
            if (defined(material.diffuseColorMap) && imagePaths.indexOf(material.diffuseColorMap) >= 0) {
                imagePaths.push(material.diffuseColorMap);
            }
            if (defined(material.emissionColorMap) && imagePaths.indexOf(material.emissionColorMap) >= 0) {
                imagePaths.push(material.emissionColorMap);
            }
            if (defined(material.specularColorMap) && imagePaths.indexOf(material.specularColorMap) >= 0) {
...
function loadResources(objPath, resourcePaths, loadFunction) {
    ...
    return Promise.map(resourcePaths, function(resourcePath) {
        if (!path.isAbsolute(resourcePath)) {
            resourcePath = path.join(path.dirname(objPath), resourcePath);
        }
